...
typedef struct __mavlink_acfly_regeisterpossensor_t {
 ...
 float LTTrustXY; /*<  Sensor long-term trust in meters in XY direction*/
 float LTTrustZ; /*<  Sensor long-term trust in meters in Z direction*/
...
#define MAVLINK_MSG_ID_ACFLY_REGEISTERPOSSENSOR_LEN 42
#define MAVLINK_MSG_ID_ACFLY_REGEISTERPOSSENSOR_MIN_LEN 42
#define MAVLINK_MSG_ID_208_LEN 42
#define MAVLINK_MSG_ID_208_MIN_LEN 42

#define MAVLINK_MSG_ID_ACFLY_REGEISTERPOSSENSOR_CRC 201
#define MAVLINK_MSG_ID_208_CRC 201
...
#define MAVLINK_MESSAGE_INFO_ACFLY_REGEISTERPOSSENSOR { \
    ...
    12, \
    {  { "target_system", NULL, MAVLINK_TYPE_UINT8_T, 0, 20, offsetof(mavlink_acfly_regeisterpossensor_t, target_system) }, \
         { "target_component", NULL, MAVLINK_TYPE_UINT8_T, 0, 21, offsetof(mavlink_acfly_regeisterpossensor_t, target_component) }, \
         { "sensor_name", NULL, MAVLINK_TYPE_CHAR, 16, 22, offsetof(mavlink_acfly_regeisterpossensor_t, sensor_name) }, \
         { "ind", NULL, MAVLINK_TYPE_UINT8_T, 0, 38, offsetof(mavlink_acfly_regeisterpossensor_t, ind) }, \
         { "type", NULL, MAVLINK_TYPE_UINT8_T, 0, 39, offsetof(mavlink_acfly_regeisterpossensor_t, type) }, \
         { "DataFrame", NULL, MAVLINK_TYPE_UINT8_T, 0, 40, offsetof(mavlink_acfly_regeisterpossensor_t, DataFrame) }, \
         { "DataType", NULL, MAVLINK_TYPE_UINT8_T, 0, 41, offsetof(mavlink_acfly_regeisterpossensor_t, DataType) }, \
         ...
         { "LTTrustXY", NULL, MAVLINK_TYPE_FLOAT, 0, 12, offsetof(mavlink_acfly_regeisterpossensor_t, LTTrustXY) }, \
         { "LTTrustZ", NULL, MAVLINK_TYPE_FLOAT, 0, 16, offsetof(mavlink_acfly_regeisterpossensor_t, LTTrustZ) }, \
         } \
}
#else
#define MAVLINK_MESSAGE_INFO_ACFLY_REGEISTERPOSSENSOR { \
 ...
 * @param LTTrustXY  Sensor long-term trust in meters in XY direction
 * @param LTTrustZ  Sensor long-term trust in meters in Z direction
...
static inline uint16_t mavlink_msg_acfly_regeisterpossensor_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
                               uint8_t target_system, uint8_t target_component, const char *sensor_name, uint8_t ind, uint8_t type, uint8_t DataFrame, uint8_t DataType, float delay, float trustXY, float trustZ, float LTTrustXY, float LTTrustZ)
{
    ...
    _mav_put_float(buf, 12, LTTrustXY);
    _mav_put_float(buf, 16, LTTrustZ);
    _mav_put_uint8_t(buf, 20, target_system);
    _mav_put_uint8_t(buf, 21, target_component);
    _mav_put_uint8_t(buf, 38, ind);
    _mav_put_uint8_t(buf, 39, type);
    _mav_put_uint8_t(buf, 40, DataFrame);
    _mav_put_uint8_t(buf, 41, DataType);
    _mav_put_char_array(buf, 22, sensor_name, 16);
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_acfly_regeisterpossensor_pack_status(uint8_t system_id, uint8_t component_id, mavlink_status_t *_status, mavlink_message_t* msg,
                               uint8_t target_system, uint8_t target_component, const char *sensor_name, uint8_t ind, uint8_t type, uint8_t DataFrame, uint8_t DataType, float delay, float trustXY, float trustZ, float LTTrustXY, float LTTrustZ)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_ACFLY_REGEISTERPOSSENSOR_LEN];
    ...
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_ACFLY_REGEISTERPOSSENSOR_MIN_LEN, MAVLINK_MSG_ID_ACFLY_REGEISTERPOSSENSOR_LEN, MAVLINK_MSG_ID_ACFLY_REGEISTERPOSSENSOR_CRC);
#else
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_ACFLY_REGEISTERPOSSENSOR_MIN_LEN, MAVLINK_MSG_ID_ACFLY_REGEISTERPOSSENSOR_LEN);
#endif
}
...
/**
 * @brief Encode a acfly_regeisterpossensor struct with provided status structure
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param status MAVLink status structure
 ...
 * @param acfly_regeisterpossensor C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_acfly_regeisterpossensor_encode_status(uint8_t system_id, uint8_t component_id, mavlink_status_t* _status, mavlink_message_t* msg, const mavlink_acfly_regeisterpossensor_t* acfly_regeisterpossensor)
{
    return mavlink_msg_acfly_regeisterpossensor_pack_status(system_id, component_id, _status, msg,  acfly_regeisterpossensor->target_system, acfly_regeisterpossensor->target_component, acfly_regeisterpossensor->sensor_name, acfly_regeisterpossensor->ind, acfly_regeisterpossensor->type, acfly_regeisterpossensor->DataFrame, acfly_regeisterpossensor->DataType, acfly_regeisterpossensor->delay, acfly_regeisterpossensor->trustXY, acfly_regeisterpossensor->trustZ, acfly_regeisterpossensor->LTTrustXY, acfly_regeisterpossensor->LTTrustZ);
}
...
static inline void mavlink_msg_acfly_regeisterpossensor_send(mavlink_channel_t chan, uint8_t target_system, uint8_t target_component, const char *sensor_name, uint8_t ind, uint8_t type, uint8_t DataFrame, uint8_t DataType, float delay, float trustXY, float trustZ, float LTTrustXY, float LTTrustZ)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_ACFLY_REGEISTERPOSSENSOR_LEN];
    _mav_put_float(buf, 0, delay);
 ...
 */
static inline void mavlink_msg_acfly_regeisterpossensor_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint8_t target_system, uint8_t target_component, const char *sensor_name, uint8_t ind, uint8_t type, uint8_t DataFrame, uint8_t DataType, float delay, float trustXY, float trustZ, float LTTrustXY, float LTTrustZ)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
...
{
    return _MAV_RETURN_char_array(msg, sensor_name, 16,  22);
}
 ...
 */
static inline uint8_t mavlink_msg_acfly_regeisterpossensor_get_ind(const mavlink_message_t* msg)
{
...
/**
 * @brief Get field LTTrustXY from acfly_regeisterpossensor message
 *
 * @return  Sensor long-term trust in meters in XY direction
 */
static inline float mavlink_msg_acfly_regeisterpossensor_get_LTTrustXY(const mavlink_message_t* msg)
{
    return _MAV_RETURN_float(msg,  12);
}

/**
 * @brief Get field LTTrustZ from acfly_regeisterpossensor message
 *
 * @return  Sensor long-term trust in meters in Z direction
 */
static inline float mavlink_msg_acfly_regeisterpossensor_get_LTTrustZ(const mavlink_message_t* msg)
{
...
static inline void mavlink_msg_acfly_regeisterpossensor_decode(const mavlink_message_t* msg, mavlink_acfly_regeisterpossensor_t* acfly_regeisterpossensor)
{
    ...
    acfly_regeisterpossensor->LTTrustXY = mavlink_msg_acfly_regeisterpossensor_get_LTTrustXY(msg);
    acfly_regeisterpossensor->LTTrustZ = mavlink_msg_acfly_regeisterpossensor_get_LTTrustZ(msg);
