...
#define MAVLINK_MSG_ID_ACFLYPOSSENSOR_INFO_CRC 220
#define MAVLINK_MSG_ID_206_CRC 220
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_acflypossensor_info_pack_status(uint8_t system_id, uint8_t component_id, mavlink_status_t *_status, mavlink_message_t* msg,
                               uint64_t time_usec, const char *sensor_name, int8_t ind, uint8_t type, uint8_t DataFrame, uint8_t DataType, double posE, double posN, double posU, float velE, float velN, float velU, int32_t Lat, int32_t Lon, float delay, float trustXY, float trustZ, const uint8_t *additionInfo1, const float *additionInfo2)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_ACFLYPOSSENSOR_INFO_LEN];
    ...
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_ACFLYPOSSENSOR_INFO_MIN_LEN, MAVLINK_MSG_ID_ACFLYPOSSENSOR_INFO_LEN, MAVLINK_MSG_ID_ACFLYPOSSENSOR_INFO_CRC);
#else
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_ACFLYPOSSENSOR_INFO_MIN_LEN, MAVLINK_MSG_ID_ACFLYPOSSENSOR_INFO_LEN);
#endif
}
...
/**
 * @brief Encode a acflypossensor_info struct with provided status structure
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param status MAVLink status structure
 ...
 * @param acflypossensor_info C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_acflypossensor_info_encode_status(uint8_t system_id, uint8_t component_id, mavlink_status_t* _status, mavlink_message_t* msg, const mavlink_acflypossensor_info_t* acflypossensor_info)
{
    return mavlink_msg_acflypossensor_info_pack_status(system_id, component_id, _status, msg,  acflypossensor_info->time_usec, acflypossensor_info->sensor_name, acflypossensor_info->ind, acflypossensor_info->type, acflypossensor_info->DataFrame, acflypossensor_info->DataType, acflypossensor_info->posE, acflypossensor_info->posN, acflypossensor_info->posU, acflypossensor_info->velE, acflypossensor_info->velN, acflypossensor_info->velU, acflypossensor_info->Lat, acflypossensor_info->Lon, acflypossensor_info->delay, acflypossensor_info->trustXY, acflypossensor_info->trustZ, acflypossensor_info->additionInfo1, acflypossensor_info->additionInfo2);
}
