...
typedef struct __mavlink_acfly_updatepossensor_t {
 ...
 float LTTrustXY; /*<  Sensor long-term trust in meters in XY direction. Set to -1 if value not changed.*/
 float LTTrustZ; /*<  Sensor long-term trust in meters in Z direction. Set to -1 if value not changed.*/
 uint8_t target_system; /*<  System ID*/
 uint8_t target_component; /*<  Component ID*/
 uint8_t ind; /*<  Position sensor index.*/
 int8_t DataType; /*<  Position sensor data type. Set to -1 if value not changed.*/
 uint8_t reset; /*<  bit7-0=available 1=not available  bit6:0-Reset counter should be increased if sensor data jumped(SLAM frame changed).*/
...
#define MAVLINK_MSG_ID_ACFLY_UPDATEPOSSENSOR_LEN 77
#define MAVLINK_MSG_ID_ACFLY_UPDATEPOSSENSOR_MIN_LEN 61
#define MAVLINK_MSG_ID_209_LEN 77
#define MAVLINK_MSG_ID_209_MIN_LEN 61

#define MAVLINK_MSG_ID_ACFLY_UPDATEPOSSENSOR_CRC 14
#define MAVLINK_MSG_ID_209_CRC 14
...
#define MAVLINK_MESSAGE_INFO_ACFLY_UPDATEPOSSENSOR { \
    ...
    17, \
    {  { "target_system", NULL, MAVLINK_TYPE_UINT8_T, 0, 56, offsetof(mavlink_acfly_updatepossensor_t, target_system) }, \
         { "target_component", NULL, MAVLINK_TYPE_UINT8_T, 0, 57, offsetof(mavlink_acfly_updatepossensor_t, target_component) }, \
         { "ind", NULL, MAVLINK_TYPE_UINT8_T, 0, 58, offsetof(mavlink_acfly_updatepossensor_t, ind) }, \
         { "DataType", NULL, MAVLINK_TYPE_INT8_T, 0, 59, offsetof(mavlink_acfly_updatepossensor_t, DataType) }, \
         ...
         { "LTTrustXY", NULL, MAVLINK_TYPE_FLOAT, 0, 48, offsetof(mavlink_acfly_updatepossensor_t, LTTrustXY) }, \
         { "LTTrustZ", NULL, MAVLINK_TYPE_FLOAT, 0, 52, offsetof(mavlink_acfly_updatepossensor_t, LTTrustZ) }, \
         { "reset", NULL, MAVLINK_TYPE_UINT8_T, 0, 60, offsetof(mavlink_acfly_updatepossensor_t, reset) }, \
         { "AttQuat", NULL, MAVLINK_TYPE_FLOAT, 4, 61, offsetof(mavlink_acfly_updatepossensor_t, AttQuat) }, \
         } \
}
#else
#define MAVLINK_MESSAGE_INFO_ACFLY_UPDATEPOSSENSOR { \
 ...
 * @param LTTrustXY  Sensor long-term trust in meters in XY direction. Set to -1 if value not changed.
 * @param LTTrustZ  Sensor long-term trust in meters in Z direction. Set to -1 if value not changed.
 * @param reset  bit7-0=available 1=not available  bit6:0-Reset counter should be increased if sensor data jumped(SLAM frame changed).
...
static inline uint16_t mavlink_msg_acfly_updatepossensor_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
                               uint8_t target_system, uint8_t target_component, uint8_t ind, int8_t DataType, double posX, double posY, double posZ, float velX, float velY, float velZ, float delay, float trustXY, float trustZ, float LTTrustXY, float LTTrustZ, uint8_t reset, const float *AttQuat)
{
    ...
    _mav_put_float(buf, 48, LTTrustXY);
    _mav_put_float(buf, 52, LTTrustZ);
    _mav_put_uint8_t(buf, 56, target_system);
    _mav_put_uint8_t(buf, 57, target_component);
    _mav_put_uint8_t(buf, 58, ind);
    _mav_put_int8_t(buf, 59, DataType);
    _mav_put_uint8_t(buf, 60, reset);
    _mav_put_float_array(buf, 61, AttQuat, 4);
    ...
    packet.LTTrustXY = LTTrustXY;
    packet.LTTrustZ = LTTrustZ;
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_acfly_updatepossensor_pack_status(uint8_t system_id, uint8_t component_id, mavlink_status_t *_status, mavlink_message_t* msg,
                               uint8_t target_system, uint8_t target_component, uint8_t ind, int8_t DataType, double posX, double posY, double posZ, float velX, float velY, float velZ, float delay, float trustXY, float trustZ, float LTTrustXY, float LTTrustZ, uint8_t reset, const float *AttQuat)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_ACFLY_UPDATEPOSSENSOR_LEN];
    ...
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_ACFLY_UPDATEPOSSENSOR_MIN_LEN, MAVLINK_MSG_ID_ACFLY_UPDATEPOSSENSOR_LEN, MAVLINK_MSG_ID_ACFLY_UPDATEPOSSENSOR_CRC);
#else
    return mavlink_finalize_message_buffer(msg, system_id, component_id, _status, MAVLINK_MSG_ID_ACFLY_UPDATEPOSSENSOR_MIN_LEN, MAVLINK_MSG_ID_ACFLY_UPDATEPOSSENSOR_LEN);
#endif
}
...
/**
 * @brief Encode a acfly_updatepossensor struct with provided status structure
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param status MAVLink status structure
 ...
 * @param acfly_updatepossensor C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_acfly_updatepossensor_encode_status(uint8_t system_id, uint8_t component_id, mavlink_status_t* _status, mavlink_message_t* msg, const mavlink_acfly_updatepossensor_t* acfly_updatepossensor)
{
    return mavlink_msg_acfly_updatepossensor_pack_status(system_id, component_id, _status, msg,  acfly_updatepossensor->target_system, acfly_updatepossensor->target_component, acfly_updatepossensor->ind, acfly_updatepossensor->DataType, acfly_updatepossensor->posX, acfly_updatepossensor->posY, acfly_updatepossensor->posZ, acfly_updatepossensor->velX, acfly_updatepossensor->velY, acfly_updatepossensor->velZ, acfly_updatepossensor->delay, acfly_updatepossensor->trustXY, acfly_updatepossensor->trustZ, acfly_updatepossensor->LTTrustXY, acfly_updatepossensor->LTTrustZ, acfly_updatepossensor->reset, acfly_updatepossensor->AttQuat);
}
...
static inline void mavlink_msg_acfly_updatepossensor_send(mavlink_channel_t chan, uint8_t target_system, uint8_t target_component, uint8_t ind, int8_t DataType, double posX, double posY, double posZ, float velX, float velY, float velZ, float delay, float trustXY, float trustZ, float LTTrustXY, float LTTrustZ, uint8_t reset, const float *AttQuat)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_ACFLY_UPDATEPOSSENSOR_LEN];
    _mav_put_double(buf, 0, posX);
 ...
 */
static inline void mavlink_msg_acfly_updatepossensor_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint8_t target_system, uint8_t target_component, uint8_t ind, int8_t DataType, double posX, double posY, double posZ, float velX, float velY, float velZ, float delay, float trustXY, float trustZ, float LTTrustXY, float LTTrustZ, uint8_t reset, const float *AttQuat)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
 ...
 */
static inline uint8_t mavlink_msg_acfly_updatepossensor_get_ind(const mavlink_message_t* msg)
{
...
{
    return _MAV_RETURN_int8_t(msg,  59);
}
...
/**
 * @brief Get field LTTrustXY from acfly_updatepossensor message
 *
 * @return  Sensor long-term trust in meters in XY direction. Set to -1 if value not changed.
 */
static inline float mavlink_msg_acfly_updatepossensor_get_LTTrustXY(const mavlink_message_t* msg)
{
    return _MAV_RETURN_float(msg,  48);
}

/**
 * @brief Get field LTTrustZ from acfly_updatepossensor message
 *
 * @return  Sensor long-term trust in meters in Z direction. Set to -1 if value not changed.
 */
static inline float mavlink_msg_acfly_updatepossensor_get_LTTrustZ(const mavlink_message_t* msg)
{
 ...
 *
 * @return  bit7-0=available 1=not available  bit6:0-Reset counter should be increased if sensor data jumped(SLAM frame changed).
 */
...
{
    return _MAV_RETURN_float_array(msg, AttQuat, 4,  61);
}
...
static inline void mavlink_msg_acfly_updatepossensor_decode(const mavlink_message_t* msg, mavlink_acfly_updatepossensor_t* acfly_updatepossensor)
{
    ...
    acfly_updatepossensor->LTTrustXY = mavlink_msg_acfly_updatepossensor_get_LTTrustXY(msg);
    acfly_updatepossensor->LTTrustZ = mavlink_msg_acfly_updatepossensor_get_LTTrustZ(msg);
