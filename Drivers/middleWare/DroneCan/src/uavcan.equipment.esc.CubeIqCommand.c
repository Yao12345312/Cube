...
uint32_t _uavcan_equipment_esc_CubeIqCommand_encode(struct uavcan_equipment_esc_CubeIqCommand* msg, uint8_t* buffer
    ...
    uint32_t bit_ofs = 0;
    memset(buffer, 0, UAVCAN_EQUIPMENT_ESC_CUBEIQCOMMAND_MAX_SIZE);
    __uavcan_equipment_esc_CubeIqCommand_encode(buffer, &bit_ofs, msg,
#if CANARD_ENABLE_TAO_OPTION
    tao
#else
    true
...
#if CANARD_ENABLE_TAO_OPTION
    if (transfer->tao && (transfer->payload_len > UAVCAN_EQUIPMENT_ESC_CUBEIQCOMMAND_MAX_SIZE)) {
        return true; /* invalid payload length */
    }
#endif
    uint32_t bit_ofs = 0;
    if (__uavcan_equipment_esc_CubeIqCommand_decode(transfer, &bit_ofs, msg,
#if CANARD_ENABLE_TAO_OPTION
    transfer->tao
#else
    true
...
struct uavcan_equipment_esc_CubeIqCommand sample_uavcan_equipment_esc_CubeIqCommand_msg(void) {
    ...
    msg.arm = (uint8_t)random_bitlen_unsigned_val(2);
    ...
    msg.Iq.len = (uint8_t)random_range_unsigned_val(0, 20);
    for (size_t i=0; i < msg.Iq.len; i++) {




        msg.Iq.data[i] = (int32_t)random_bitlen_signed_val(18);
